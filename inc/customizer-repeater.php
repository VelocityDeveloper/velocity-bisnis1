...
class Velocity_SmallBanner_Repeater_Control extends WP_Customize_Control {
    ...
    public function render_content() {
        ...
        foreach ($items as $item) {
            ...
            echo '<div class="vr-body">';
            echo '<label>Banner Image</label><small class="description">Ukuran 380x380</small>';
            echo '<div class="vr-preview">'.($thumb ? '<img src="'.$thumb.'" />' : '').'</div>';
            echo '<input class="vr-img-id" type="hidden" value="'.$id.'">';
            echo '<div class="vr-button"><button type="button" class="button-link vr-remove">Remove</button> <button type="button" class="button button-secondary vr-select">Select Image</button></div>';
        ...
        echo '<input type="hidden" class="vr-hidden" value="'. esc_attr($value) .'">';
        echo <<<JS
<script>(function($){var root=$('.velocity-smallbanner-repeater').last(),add=root.nextAll('.vr-add').first(),hidden=add.nextAll('.vr-hidden').first();var limit=parseInt(root.attr('data-limit'))||3;function collect(){var items=[];root.find('.vr-item').each(function(){var id=$(this).find('.vr-img-id').val();var title=$(this).find('.vr-title-input').val();if(id||title){items.push({id:id?parseInt(id):0,title:title||''});}});var json=JSON.stringify(items);hidden.val(json);try{if(window.wp&&wp.customize){wp.customize("{$this->id}",function(s){if(s&&s.set){s.set(json);}});}}catch(e){}}root.on('click','.vr-head',function(){var item=$(this).closest('.vr-item');item.find('.vr-body').slideToggle(150);item.toggleClass('collapsed');});root.on('input change','.vr-title-input',function(){var v=$(this).val();var item=$(this).closest('.vr-item');var t=item.find('.vr-title');if(v){t.text(v);}else{var idx=item.index()+1;t.text('row '+idx);}collect();});root.on('click','.vr-remove',function(){ $(this).closest('.vr-item').remove(); collect(); });root.on('click','.vr-select',function(){var item=$(this).closest('.vr-item'),idInput=item.find('.vr-img-id'),preview=item.find('.vr-preview');var lib=wp.media.query({type:'image'});var frame=wp.media({title:'Select Image',library:lib,multiple:false});frame.on('select',function(){var att=frame.state().get('selection').first().toJSON();idInput.val(att.id);preview.html('<img src="'+(att.sizes&&att.sizes.thumbnail?att.sizes.thumbnail.url:att.url)+'" />');collect();});frame.open();});add.on('click',function(){if(root.find('.vr-item').length>=limit){alert('Limit reached');return;}var count=root.find('.vr-item').length+1;root.append('<div class="vr-item"><div class="vr-head"><strong class="vr-title">row '+count+'</strong><span class="vr-caret"></span></div><div class="vr-body"><label>Banner Image</label><br/><small class="description">Ukuran 380x380</small><div class="vr-preview"></div><input class="vr-img-id" type="hidden" value=""><div class="vr-button"><button type="button" class="button button-secondary vr-select">Select Image</button><button type="button" class="button-link vr-remove">Remove</button></div><label class="mt-3">Banner Title</label><input type="text" class="vr-title-input" value=""></div></div>');collect();});})(jQuery);</script>
JS;
    }
}
}

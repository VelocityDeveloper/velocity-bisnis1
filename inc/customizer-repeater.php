...
class Velocity_Media_Repeater_Control extends WP_Customize_Control {
    ...
    public function render_content() {
        ...
        foreach ($items as $item) {
            ...
            echo '<div class="vr-button"><button type="button" class="button button-secondary vr-select">Select Image</button> <button type="button" class="button remove-button vr-remove">Remove</button></div>';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';
...
<script>(function($){
  ...
  function bindAll(){
    root.find('.vr-content').each(function(){ bindMceFor($(this).attr('id')); });
  }
  $(document).on('click','.media-modal-close',function(){ setTimeout(bindAll, 100); });
...
class Velocity_SmallBanner_Repeater_Control extends WP_Customize_Control {
    ...
    public function render_content() {
        ...
        foreach ($items as $item) {
            ...
            echo '<div class="vr-preview">'.($thumb ? '<img src="'.$thumb.'" />' : '').'</div>';
            echo '<input class="vr-img-id" type="hidden" value="'.$id.'">';
            echo '<div class="vr-button"><button type="button" class="button remove-button vr-remove">Remove</button> <button type="button" class="button button-secondary vr-select">Select Image</button></div>';
            echo '<label class="mt-3"><b>Banner Title</b></label><input type="text" class="vr-title-input" value="'.$title.'">';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';

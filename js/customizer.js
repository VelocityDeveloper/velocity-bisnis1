;(function($){
  ...
  function initServices(container){
    ...
    container.on('click','.vr-head',function(){var item=$(this).closest('.vr-item'),body=item.find('.vr-body'),ta=item.find('.vr-content'),id=ta.attr('id');body.toggle();if(body.is(':visible')){try{if(window.wp&&wp.editor&&(!window.tinymce||!tinymce.get(id))){wp.editor.initialize(id,{tinymce:true,quicktags:true});}}catch(e){}bindEditor(id);}item.toggleClass('collapsed');});
  ...
  function initMedia(container){
    ...
    var scheduleCollect=debounce(collect,150);
    container.on('click','.vr-head',function(){var item=$(this).closest('.vr-item');item.find('.vr-body').toggle();item.toggleClass('collapsed');});
  ...
  function bindAllEditors(){ $('.velocity-repeater .vr-content').each(function(){var id=$(this).attr('id');var ed=(window.tinymce?tinymce.get(id):null);if(ed&&!ed._vrBound){ed.on('change keyup input NodeChange', function(){ $('#'+id).trigger('keyup'); });ed._vrBound=true;}}); }
  ...
  $(function(){
    ...
    $(document).on('click','.media-modal-close',function(){setTimeout(bindAllEditors,100);});
    setTimeout(bindAllEditors,100);
  });
})(jQuery);